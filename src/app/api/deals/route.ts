  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      for (const deal of body) {
        const result = await updateDeal(deal, dealRepository);
    ...
    // Single update
    const result = await updateDeal(body, dealRepository);
    if (result.success) {
      return NextResponse.json({ deal_id: result.deal_id }, { status: 200 });
    } else {
